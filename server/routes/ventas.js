...
import { verificarStockBajo, notificarVenta } from "../middleware/notificaciones-automaticas.js"
...
router.post("/", authenticateToken, async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      await verificarStockBajo(item.producto_id)
    ...
    // 🆕 NOTIFICAR VENTA IMPORTANTE (si es mayor a $10,000)
    await notificarVenta(venta_id, total, req.user.nombre)
...
router.put("/:id/anular", authenticateToken, async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      // 🆕 VERIFICAR STOCK DESPUÉS DE RESTAURAR (por si ya no está bajo)
      await verificarStockBajo(item.producto_id)
